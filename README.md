...
- **v1.1 — timestamps** (shipped 2026-07-20): framed packets + clock-sync handshake; prime-once + depth-steering controller. Honest claim per review: **restart-stable sync in the tested configurations** (real-device bench: 3× restart, no re-tune) — NOT a general fixed-latency guarantee; stall/gap recovery VERIFIED under a deterministic virtual-time disturbance envelope (tests/test_controller_virtual.py: stalls 0.5/3/8s, pause-gaps, +-30s skew, sink saturation, DAC drift; sub-guard stalls recover in ~2 drain episodes ~4.5s; large-sink drift blindness documented as a known limit with a failing-test-ready case). (A ±2.4 ms figure previously cited here belonged to the rejected v1.0 controller.) Spec: `docs/WIRE-FORMAT.md`.
